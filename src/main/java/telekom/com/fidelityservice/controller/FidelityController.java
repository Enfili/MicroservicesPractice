...
public class FidelityController {
    ...
    public ResponseEntity<UserDTO> updateSpentMoney(@RequestParam int userId, @RequestParam double productPrice) {
        ...
        double productPriceAfterDiscount = fidelityService.calculateDiscount(foundUser.get().getMoneySpent(), productPrice);
        foundUser.get().setMoneySpent(foundUser.get().getMoneySpent() + productPriceAfterDiscount);
        Optional<User> updatedUser = fidelityService.save(foundUser.get());
